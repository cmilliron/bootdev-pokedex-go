...
func StartRepl(cfg *Config) {
	...
	for {
		...
		if ok {
			err := replCommand.callback(cfg, args...)
			if err != nil {
				fmt.Println(err)
			}
			continue
...
func getCommands() map[string]cliCommand {
	return map[string]cliCommand {
		...
		"explore": {
			name: "explore",
			description: "Explore details of a given location",
			callback: handleExploreCommand,
		},
		"catch": {
			name: "catch",
			description: "Catch one pokemon. So you can have them all",
		...
		"pokedex": {
			name: "pokedex",
			description: "Dipsplay all poke-persons in your Pokedex",
